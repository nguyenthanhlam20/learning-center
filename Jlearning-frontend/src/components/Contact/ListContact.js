...
const ListContact = ({ data }) => {
  ...
  const handlePageChange = (value) => {
    setPage(value);
    setContactsPagination(
      contacts.slice(value * rowsPerPage, value * rowsPerPage + rowsPerPage)
    );
  };
  ...
  React.useEffect(() => {
    setResponseMessage(
      currentContact?.response_message ? currentContact?.response_message : ""
    );
  }, [currentContact]);
  ...
  return (
    <>
      ...
      >
        <DialogTitle>
          Gửi Phản Hồi Tới Địa Chỉ Email: {currentContact?.email}
        </DialogTitle>
        <DialogContent
          sx={{
            ...
            <Button
              ...
              title={
                currentContact?.email != null
                  ? "Gửi phản hồi tới " + currentContact?.email
                  : "Gửi phản hồi"
              }
