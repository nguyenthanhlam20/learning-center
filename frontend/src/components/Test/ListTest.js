...
const ListTest = ({ data, courses }) => {
  ...
  const handlePageChange = (value) => {
    setPage(value);
    setTestsPagination(
      tests.slice(value * rowsPerPage, value * rowsPerPage + rowsPerPage)
    );
  };
  ...
  React.useEffect(() => {
    const result = data.filter((test) =>
      test.test_name.toLowerCase().includes(searchTerm.value.toLowerCase())
    );
    setTests(result);
    setPage(0);
    setRowsPerPage(5);
